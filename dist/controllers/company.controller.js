...
exports.companyController = {
    async getProfile(req, res) {
        try {
            ...
            if (req.user?.role !== 'COMPANY') {
                throw new error_middleware_1.AppError(403, 'Only company accounts can access this resource');
            }
    ...
    async createJob(req, res) {
        try {
            ...
            if (!company) {
                throw new error_middleware_1.AppError(404, 'Company profile not found. Please create a company profile first.');
            }
            const { title, description, location, salary, employmentType, experienceLevel, remote, requirements, benefits, category, status = 'OPEN' } = req.body;
            // Validate required fields
            if (!title || !description || !location || !employmentType || !experienceLevel || !category) {
                throw new error_middleware_1.AppError(400, 'Missing required job fields');
            }
    ...
    async updateJob(req, res) {
        try {
            ...
            if (!job) {
                throw new error_middleware_1.AppError(404, 'Job not found');
            }
    ...
    async getStats(req, res) {
        try {
            ...
            const newApplications = company.jobs.reduce((acc, job) => acc + job.applications.filter(app => app.status === 'PENDING').length, 0);
            return res.json({
                success: true,
                data: {
                    activeJobs,
